...
quest :-
    ...
    CurGold is (Gold + (PriceFarm * (CountFarm + 1) + (PriceFish * (CountFish + 1)) + (PriceRanch * (CountRanch + 1)) + 20)),
    CurExp is Exp + (((5 * Level) * (CountFarm + CountFish + CountRanch))),
    retractall(player(_, _, _, _, _, _, _, _, _, _, _, _)),
    assertz(player(A, B, C, D, E, F, G, H, CurExp, J, CurGold, L)),
    format('You got ~w Golds and ~w Exp!', [CurGold, CurExp]),
